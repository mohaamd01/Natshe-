...
interface ProductCardProps {
  product: Product;
  /** If true, card uses a taller aspect ratio (4/5). Default: true */
  tall?: boolean;
  /** Mark the hero image as priority (LCP above-the-fold images). Default: false */
  priority?: boolean;
}
...
export default function ProductCard({ product, tall = true, priority = false }: ProductCardProps) {
  ...
  return (
    <article className="group flex flex-col">
       ...
       * Stretched <Link> (z-10) covers the image for navigation.
       * WhatsApp <a> (z-20) is a sibling of that Link, never nested inside it.
       * This prevents the <a>-inside-<a> hydration error.
       */}
      <div
        className="relative overflow-hidden rounded-2xl bg-ivory-dark"
        style={{ aspectRatio: tall ? "4/5" : "1/1" }}
      >
        <Image
          src={heroImage.src}
          alt={heroImage.alt}
        ...
        {/* Quick-order WhatsApp overlay — z-20, sibling of the Link, never nested inside it */}
