          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            <StaggerReveal stagger={80}>
              {crystals.map((crystal) => {
                ...
                const chakra = localized(crystal.properties.chakra, locale);
                const idealFor = localized(crystal.properties.idealFor, locale);
                ...
                return (
                  ...
                  >
                    <div className="relative overflow-hidden" style={{ aspectRatio: "4/3" }}>
                      <Image src={crystal.thumbnail} alt={`${name} crystal jewelry`} fill className="object-cover object-center transition-transform duration-700 ease-luxury group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                    ...
                    <div className="flex flex-col flex-1 p-5 md:p-6">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <p className="font-sans text-[10px] tracking-[0.2em] uppercase text-brown/40">{chakra} {t("chakraLabel")}</p>
                      </div>
                      <h2 className="font-serif font-light text-brown mb-1 leading-tight" style={{ fontSize: "clamp(1.3rem, 2.5vw, 1.7rem)" }}>{name}</h2>
                      <p className="font-sans text-[11px] tracking-[0.1em] text-sage uppercase font-medium mb-3">{subtitle}</p>
                      <p className="font-sans text-sm text-brown/55 leading-relaxed line-clamp-3 flex-1">{summary}</p>
                      <div className="flex items-center justify-between mt-4 pt-4 border-t border-brown/8">
                        <span className="font-sans text-xs text-brown/35">{idealFor.split(",")[0].trim()}</span>
